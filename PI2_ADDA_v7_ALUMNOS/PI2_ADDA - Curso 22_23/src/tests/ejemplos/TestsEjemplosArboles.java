...
public class TestsEjemplosArboles {
	...
	public static void testsEjemplo4() {
		...
		List<Pair<BinaryTree<Character>, List<Character>>> inputs = Files2.streamFromFile(file).map(linea -> {
			...
			Preconditions.checkArgument(linea.length() == 2);
	...
	public static void testsEjemplo5() {
		...
		List<Tree<Integer>> inputs = Files2.streamFromFile(file).map(linea-> Tree.parse(linea,s-> Integer.parseInt(s))).toList();
		...
		inputs.stream().forEach(x-> System.out.println( x + ":"+Ejemplo5.solucion_recursiva(x,PRIMO)));
	}
