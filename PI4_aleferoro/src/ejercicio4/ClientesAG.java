...
public class ClientesAG implements SeqNormalData<SolucionClientes> {
	public ClientesAG(String fichero) {
		...
		for (int i = 0; i < value.size(); i++) {
			...
			if (i == 0) {
				if (DatosClientes.existeArista(0, value.get(i))) {
					...
					if (DatosClientes.getCliente(value.get(i)).id() != 2) {
						goal += DatosClientes.getBeneficio(value.get(i)) - suma;
					} else {
						goal += DatosClientes.getBeneficio(value.get(i));
					}
